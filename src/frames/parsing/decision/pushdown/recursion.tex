...
\begin{frame}[fragile]{Algorithme de reconnaissance récursif}
  ...
  \tf<2-|handout>[y=-25mm, x=-.25\textwidth]{
    \begin{tikzpicture}[smAutomaton, initial text=$\text{empile}(\diamond)$]
      ...
      \uncover<8-|handout>{
        \smState[\smAccepting] (b) at (3,0) {1};
        \smPath (a) edge node {$\diamond : \text{dépile}(\diamond)$} (b);
      }
    \end{tikzpicture}
  }

  \tfBlock<2-|handout>[y=-25mm, right=37mm]{Pile d'appels}{
