...
\begin{frame}[fragile]{Algorithme de reconnaissance récursif}
  ...
  \on[top, text]{
    \begin{algorithm}[H]
      \Fun{$\Decider(u : \text{mot}) : \text{booléen}$}{
        \Return $\Trouver( u \cdot \text{`$\blank$'}, \text{`$\blank$'} )$\;
      }
  ...
  \onExampleBlock<2-|handout>[y=5mm,right=37mm]{Exemple : $u = \{\{\}\langle\rangle\}$}{
    \begin{tikzpicture}[word, size=5mm]
      ...
      \cell{$\}$}       \smhead[ob=<7>] 
      \cell{$\blank$} \smhead[ob=<8>] 
    \end{tikzpicture}
  }

  \on<2->[y=-25mm, x=-.25\textwidth]{
    \begin{tikzpicture}[pushdown]
      \state[initial, initial text=$\text{empile}(\blank)$] (a) at (0,0) {0};
      ...
      \uncover<8->{
        ...
        \path (a) edge node {$\blank : \text{dépile}(\blank)$} (b);
      }
    \end{tikzpicture}
  }
