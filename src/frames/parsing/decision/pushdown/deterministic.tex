...
\begin{frame}{Automates à pile déterministes}
  ...
  \begin{block}{Définition -- Automate à pile déterministe}
    On dit que $A$ est \structure{déterministe}\footnote{Il existe d'autres définitions, généralement plus restrictives, dans la littérature classique.} si ces deux conditions sont vérifiées : 
    \begin{enumerate}
    \item $A$ ne possède qu'un seul état initial : \alert{$\exists q_0\in Q,~I = \{q_0\}$}
    \item Pour tout état \structure{$q \in Q$} et transitions \structure{$q\xrightarrow{\smPAtrans{a_1}{p_1}{p'_1}} q_1$}
