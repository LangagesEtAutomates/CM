...
\begin{frame}[fragile]{Algorithme Cocke-Younger-Kasami (CYK)}
  \tf[top, text]{\small
    \begin{algorithm}[H]
      \Fn{\CYK($\langle \Sigma, \Gamma, S, \rightarrow \rangle$ : grammaire, $u \in \Sigma^\star$) : booléen}{
        \lSi{$u = \varepsilon$}{\Retourner $S \rightarrow \varepsilon$}
        \Pour{$j$ allant de $1$ à $|u|$}{
          \tfAlert<3>{$T[1, j] \leftarrow \{N\in \Gamma \mid N \rightarrow u[j]\}$}\;
        }
        \Pour{$i$ allant de $2$ à $|u|$}{
          \Pour{$j$ allant de $1$ à $|u|-i+1$}{
            \tfAlert<4->{$\displaystyle T[i, j] \leftarrow \bigcup_{k=1}^{i-1} \left\{N\in \Gamma \,\middle\mid\, \exists X \in T[k,j], \exists Y \in T[i-k,j+k],~ N \rightarrow X Y\right\}$}\;
          }
        }
  ...
  \tfExampleBlock[bottom]{Exemple}{
    ...
    \alert<5,9>{S} &\alert<5,9>{\rightarrow}&  \alert<9>{AC} \mid \alert<5>{AB} \mid \varepsilon  \\
    \alert<8>{C} &\alert<8>{\rightarrow}&  \alert<8>{TB}  \\
    \alert<5,9>{T} &\alert<5,9>{\rightarrow}&  \alert<9>{AC} \mid \alert<5>{AB}  \\
    \alert<3>{A} &\alert<3>{\rightarrow}& \alert<3>{a}  \\
    \alert<3>{B} &\alert<3>{\rightarrow}& \alert<3>{b}
    \end{array}\right.\\
    u & aabb
    \end{array}$
  }
