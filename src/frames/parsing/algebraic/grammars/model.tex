...
\begin{frame}{Modélisation mathématique}
  ...
  \begin{block}{Définition -- Grammaire algébrique (ou hors-contexte)}
    \vspace{2mm}
    Une \structure{grammaire algébrique} est un quadruplet \alert{$\langle \Sigma, \Gamma, S, \rightarrow \rangle$} tel que :
    ...
    \item[\alert{$\rightarrow$}] $\subseteq \Gamma \times (\Sigma \cup \Gamma)^\star$ : \structure{l'ensemble des règles de production}
    \end{description}

    \vspace{3mm}
    Une \structure{règle de production} est un couple \alert{$\langle A, \alpha \rangle \in \rightarrow$}, noté $\alert{A \rightarrow \alpha}$ tel que :
    \begin{description}[xxxx]
