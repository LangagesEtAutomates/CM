...
\begin{frame}{Démonstration du lemme d'Arden}
  ...
  \begin{tikzpicture}
    ...
    \draw (10,10) node[below]{\begin{minipage}{\textwidth}
        ...
        \begin{enumerate}
        \item \structure{$A^\star B$ est solution de $X = AX | B$} \hspace\fill \alert{$A^\star B = A (A^\star B) \cup B$}
        \item<1,3-> \structure{$A^\star B$ est la plus petite solution de $X = AX | B$} \hspace\fill \alert{$A^\star B \subseteq \mathcal{X}$}
        \item<1,7-> \structure{Si $\varepsilon \notin A$, $A^\star B$ est la seule solution de $X = AX | B$} \hspace\fill \alert{$\varepsilon \notin A \Rightarrow A^\star B = \mathcal{X}$}
        \end{enumerate}
    \end{minipage}};

    \draw<2|handout:1> (10,7.9) node[below]{\begin{minipage}{\textwidth}
        \begin{itemize}
    ...
    \draw<3-6|handout:2> (10,7.9) node[below]{\begin{minipage}{\textwidth}
        \begin{itemize}
    ...
    \uncover<7-|handout:3>{
      \draw (10,7.9) node[below]{\begin{minipage}{\textwidth}
          \begin{itemize}
          ...
          \item<8-> \vspace{-1mm}Montrons que $\alert{\forall n\in \mathbb{N}, \mathcal{X} \cap \Sigma^n \subseteq A^\star B}$\only<9->{ par récurrence forte sur $n$}.\\
            \uncover<9->{
              ...
              \item<10-> $u \in \mathcal{X} = A\mathcal{X} | B$. Deux cas possibles \\
