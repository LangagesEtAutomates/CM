...
\begin{frame}{L'algorithme de Moore}
  ...
  \on[text,top=-2mm]{\small
    \begin{algorithm}[H]
      \Fun{\Moore( $A = \langle \Sigma, Q, I, F, \rightarrow \rangle$ : AFD ) : AFD}{
        ...
        \Repeat{
          ...
          \eIf{\Alertb<3>{$\exists \langle s, a, s_1 \rangle, \langle s, a, s_2 \rangle \in \tau,~s_1 \neq s_2$}}{
  ...
  \onExampleBlock[top=-5mm, right=.29\textwidth]{Exemple}{
    \begin{tikzpicture}[automaton, grid size=8mm]
      \state[example, structure ob=<2-3>, initial] (0) at (0,1) {$0$};
      \state[example, structure on=<2>           ] (1) at (1,2) {$1$};
      \state[example, structure on=<2>           ] (2) at (1,0) {$2$};
      \state[example, alert on=<2->, accepting   ] (3) at (2,1) {$3$};
