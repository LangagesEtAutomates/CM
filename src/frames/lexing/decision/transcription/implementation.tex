...
\begin{frame}{Implémentation d'un automate déterministe complet}
  ...
  \on[y=0mm]{\small
    \begin{algorithm}[H]
      \Fun{\Decider($u \in \Sigma^\star$, $A$ : AFN) : booléen}{
        $q \leftarrow 0$\;
        \For{$i$ \From $1$ \To $|u|$}{$q \leftarrow A.\Transition[A.\Symbols[u[i]], q]$}
        \Return $A.\Final[q]$\;
      }
    \end{algorithm}
  }
  ...
  \on[x=25mm, y=-25mm]{
    $\begin{array}{@{\;}l@{\;}c@{\;}l@{\;}}
      ...
      \begin{tikzpicture}[x=6mm, y=4mm, baseline=(center)]
        \coordinate (center) at (0,-1mm);
        \node[draw, rectangle, minimum width=6mm] (x) at (0,0) {\tiny\False}; 
        \node[draw, rectangle, minimum width=6mm] (x) at (1,0) {\tiny\False};     
        \node[draw, rectangle, minimum width=6mm] (x) at (2,0) {\tiny\False}; 
        \node[draw, rectangle, minimum width=6mm] (x) at (3,0) {\tiny\True};     
        \node[draw, rectangle, minimum width=6mm] (x) at (4,0) {\tiny\False}; 
      \end{tikzpicture}\\
    \end{array}
    $
  }
